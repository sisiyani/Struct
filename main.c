...
int main(int argc, char const *argv[]) {
  ...
  do{
  menu();
  char* s=fgets(s,sizeof(int),stdin);
  rep=atoi(s);
    switch (rep) {
      case 1:
        printf("Affichage :\n");
      ...
      case 2:
        ...
        if (fgets(ligne,550,stdin )&& sscanf(ligne,"%d %s %s",&num,titre,auteur)==3) {
      ...
      case 3:
        ...
        if (fgets(ligne,550, stdin) && sscanf(ligne,"%d ",&num)==1 ){
